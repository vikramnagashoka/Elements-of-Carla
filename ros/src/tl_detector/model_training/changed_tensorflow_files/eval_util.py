...
def evaluate_traffic_light_detection(result_lists, categories):
  """Computes traffic light detection metrics given groundtruth and detections.

  This function computes traffic light metrics. This function by default
  takes detections and groundtruth boxes encoded in result_lists and writes
  evaluation results to tf summaries which can be viewed on tensorboard.
  ...
  Args:
    ...
    categories: a list of dictionaries representing all possible categories.
      Each dict in this list has the following keys:
          'id': (required) an integer id uniquely identifying this category
          'name': (required) one of 'Green', 'Yellow', 'Red'
  ...
  labels = {x['id']:x['name'] for x in categories}
  metrics = {'Detection Error':0, 'Classification Error':0}
  
  for detection_scores, detection_classes, groundtruth_classes in \
      zip(result_lists['detection_scores'], \
          result_lists['detection_classes'], \
          result_lists['groundtruth_classes']):
      if len(detection_scores) == 0 or detection_scores[0] < 0.1:
          if len(groundtruth_classes) > 0 and labels[groundtruth_classes[0]] != 'Green':
              metrics['Detection Error'] += 1 
      elif len(groundtruth_classes) == 0 and labels[detection_classes[0]] != 'Green':
          metrics['Detection Error'] += 1
      elif len(groundtruth_classes) > 0 and detection_classes[0] != groundtruth_classes[0]:
          metrics['Classification Error'] += 1

  metrics['Combined Error'] = metrics['Classification Error'] + metrics['Detection Error']
  n_images = len(result_lists['detection_scores'])
  metrics['Classification Error'] /= n_images
  metrics['Detection Error'] /= n_images
  metrics['Combined Error'] /= n_images
...
def run_checkpoint_once(tensor_dict,
  ...
  if restore_fn:
    new_checkpoint = restore_fn(sess, last_checkpoint, checkpoint_file)
    if new_checkpoint == last_checkpoint: 
        sess.close() # no new model was found
        return
    else:
