...
EVAL_METRICS_FN_DICT = {
    ...
    'traffic_light_metrics': eval_util.evaluate_traffic_light_detection
}
...
def evaluate(create_input_dict_fn, create_model_fn, eval_config, categories,
  ...
  def _restore_latest_checkpoint(sess, last_checkpoint_num, latest_checkpoint):
    latest_checkpoint_num = int(latest_checkpoint.split('-')[-1])
    if last_checkpoint_num + 4 > latest_checkpoint_num:
        return last_checkpoint_num

    next_checkpoint_num = last_checkpoint_num + 4
    next_checkpoint = checkpoint_dir+'/model.ckpt-'+str(next_checkpoint_num)
    while next_checkpoint_num < latest_checkpoint_num:
        if os.path.exists(next_checkpoint+'.index'):
            break
        else:
            next_checkpoint_num += 1
            next_checkpoint = checkpoint_dir+'/model.ckpt-'+str(next_checkpoint_num)

    saver.restore(sess, next_checkpoint)
